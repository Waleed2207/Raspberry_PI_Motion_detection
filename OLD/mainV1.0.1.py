...
LED_PIN = 17    # Corresponds to GPIO 17
PIR_PIN = 20    # Corresponds to GPIO 20
RELAY_PIN = 19  # Corresponds to GPIO 19
...
# Devices setup
led = LED(LED_PIN)
relay = OutputDevice(RELAY_PIN, active_high=False, initial_value=True)  # Initialize Relay OFF
...
def led_relay_on():
    ...
    if not led_status:
        ...
        if server_running and send_request_to_node("on"):
            try:
                led.on()
                relay.off()  # Relay ON in active_high=False configuration
                led_status = True
                print("Bulb ON, Relay ON")
...
def led_relay_off():
    ...
    if led_status:
        led.off()
        relay.on()  # Relay OFF in active_high=False configuration
        led_status = False
        print("Bulb OFF, Relay OFF")
...
if __name__ == '__main__':
    try:
        led_relay_off()  # Ensure the LED and relay are off initially
